-- ========================================
-- QUANTITAIZER FASE 2 - ML TABLES DEPLOYMENT
...
CREATE TABLE IF NOT EXISTS ml_predictions (
  ...
  prediction_horizon INTEGER NOT NULL DEFAULT 7,
...
-- Add ML columns to fed_data table
...
BEGIN
  ...
  LEFT JOIN fed_data f ON f.date = p.date + INTERVAL '7 days'
...
-- Insert some test data for immediate testing
INSERT INTO ml_predictions (date, model_version, predicted_score, predicted_scenario, confidence) 
VALUES 
  (CURRENT_DATE, 'rule_based_v1', 55, 'Neutral', 0.85),
  (CURRENT_DATE - INTERVAL '1 day', 'rule_based_v1', 52, 'QT Moderate', 0.78)
ON CONFLICT (date, model_version, prediction_horizon) DO NOTHING;

INSERT INTO pattern_analysis (date, analysis_type, regime, regime_confidence, detected_cycles)
VALUES 
  (CURRENT_DATE, 'REGIME', 'CRISIS', 85, 1),
  (CURRENT_DATE - INTERVAL '1 day', 'REGIME', 'SCARCE', 78, 0)
ON CONFLICT (date, analysis_type) DO NOTHING;
...
UNION ALL
SELECT 
  'pattern_analysis' as table_name, 
  COUNT(*) as record_count 
FROM pattern_analysis
UNION ALL
SELECT 
  'model_performance' as table_name, 
  COUNT(*) as record_count 
FROM model_performance
UNION ALL
SELECT 
  'market_correlations' as table_name, 
  COUNT(*) as record_count 
FROM market_correlations;

-- Success message
SELECT 'ML TABLES DEPLOYED SUCCESSFULLY! 🚀' as status;
